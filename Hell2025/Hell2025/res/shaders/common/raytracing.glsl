...
bool MeshClosestHit(in uint rootNodeIndex, in uint instanceIndex, in vec3 rayOrigin, in vec3 rayDir, float minDistance, inout float maxDistance, in mat4 inverseWorldTransform, inout RayResult record) {
    ...
    // cache scale factor to avoid double shrinking maxDistance during hits
    ...
    while (stack_size != 0) {
        ...
        // check aabb
        if (!IntersectNode(localOrigin, localDirInv, localMinDistance, currentLocalMax, node.boundsMin, node.boundsMax)) {
            continue;
        }
        ...
        if (node.primitiveCount > 0) {
            for (int i = 0; i < int(node.primitiveCount); i++) {
                ...
                // test triangles
                if (IntersectTriangleClosest(localOrigin, localDir, localMinDistance, currentLocalMax, p0, e1, e2, normal, barycentrics)) {
                    hitFound = true;
                    
                    // update hit distance globally
...
// traverse scene bvh to find the nearest global intersection
bool ClosestHit(in vec3 rayOrigin, in vec3 rayDir, float minDistance, float maxDistance, out RayResult record) {
    ...
    while (stack_size != 0) {
        ...
        // narrow bounds naturally cull distant nodes as currentMaxDistance shrinks
        if (!IntersectNode(rayOrigin, rayDirInv, minDistance, currentMaxDistance, node.boundsMin, node.boundsMax)) {
            continue;
        }
        ...
        if (node.primitiveCount > 0) {
            for (int i = 0; i < int(node.primitiveCount); i++) {
                ...
                // currentMaxDistance gets updated inside MeshClosestHit if a tighter hit is found
...
bool IntersectTriangleClosestNew(in vec3 rayOrigin, in vec3 rayDir, float minDistance, inout float maxDistance, in vec3 p0, in vec3 e1, in vec3 e2, in vec3 normal, out float signedT, out vec2 barycentrics) {
    vec3 c = p0 - rayOrigin;
    vec3 r = cross(rayDir, c);
    float det = dot(normal, rayDir);
...
bool MeshClosestHitNew(in uint rootNodeIndex, in uint instanceIndex, in vec3 rayOrigin, in vec3 rayDir, float minDistance, inout float maxDistance, in mat4 inverseWorldTransform, inout RayResult record) {
    uint stack[MAX_BVH_STACK_SIZE];
    uint stack_size = 0;
    stack[stack_size++] = rootNodeIndex;
    bool hitFound = false;
    ...
    while (stack_size != 0) {
        ...
        if (node.primitiveCount > 0) {
            for (int i = 0; i < int(node.primitiveCount); i++) {
                ...
                float signedHitT;
                vec2 bary;
                if (IntersectTriangleClosestNew(localOrigin, localDir, localMinDistance, currentLocalMax, p0, e1, e2, normal, signedHitT, bary)) {
                    hitFound = true;
                    maxDistance = currentLocalMax * localToWorldScale; 
...
bool ClosestHitNew(in vec3 rayOrigin, in vec3 rayDir, float minDistance, float maxDistance, out RayResult record) {
    uint stack[MAX_BVH_STACK_SIZE];
    uint stack_size = 0;
    stack[stack_size++] = 0; 
    vec3 rayDirInv = 1.0 / rayDir;
    ...
    while (stack_size != 0) {
        ...
        if (node.primitiveCount > 0) {
            for (int i = 0; i < int(node.primitiveCount); i++) {
                uint instIndex = node.firstChildOrPrimitive + i;
                EntityInstance instance = EntityInstances[instIndex];
                if (MeshClosestHitNew(uint(instance.rootNodeIndex), instIndex, rayOrigin, rayDir, minDistance, currentMaxDistance, instance.inverseWorldTransform, record)) {
                    hitFound = true;
                }
            }
        } else {
