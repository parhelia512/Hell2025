...
void Shark::Init(glm::vec3 initialPosition) {
    ...
    // Hack in a path
    glm::vec3 center(13.74f, 30.0f, 36.0f);
    float radius = 10.0f;
    int segments = 9;
...
void Shark::Update(float deltaTime) {
    ...
    // EVERY FRAME, TARGET THE CLOSET PLAYER, OR RETURN TO PATH

    m_movementState = SharkMovementState::FOLLOWING_PATH_ANGRY;
    bool targetFound = false;
    float closestDistanceToTarget = 9999999.0f;
    ...
    if (m_movementState == SharkMovementState::FOLLOWING_PATH_ANGRY) {
        ...
        for (int i = 0; i < Game::GetLocalPlayerCount(); i++) {
            ...
            if (player) {
                ...
                // Also bail if they are kinda on the rock or whatever
                float safeHeight = Ocean::GetOceanOriginY() - 1.19f;
                bool beneathSafeHeight = (player->GetFootPosition().y < safeHeight);

                glm::vec3 playerPos2D = player->GetFootPosition() * glm::vec3(1.0f, 0.0, 1.0f);
                glm::vec3 sharPos2D = GetHeadPosition2D();

                float distanceToTarget = glm::distance(sharPos2D, playerPos2D);

                if (beneathSafeHeight && distanceToTarget < closestDistanceToTarget) {
                    ...
                    m_huntedPlayerId = player->GetPlayerId();
                    m_targetPosition = player->GetFootPosition();
                    std::cout << "TARGET FPIND!: " << m_huntedPlayerId << "\n";
                }
            }
        }
    }
    ...
    if (animatedGameObject) {
        ...
        // this will cause problems later for sure, LOOK HERE!!!!!!!!!!!
        if (IsAlive()) {
